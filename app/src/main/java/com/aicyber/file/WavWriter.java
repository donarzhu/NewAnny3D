...
public class WavWriter {
	...
	/** member properties */
	private RandomAccessFile mInternalWriter;//文件操作对象
	private short mNumChannels;       //通道数量
	private int mSampleRate;          //采样率
	private short mBitsPerSample;     //比特率
	 ...
	 * @throws IOException
	 */
	public void writeHeader(int len) throws IOException {
		/* RIFF header */
	...
	{
		try {
			long  size = mInternalWriter.length();
			if (size > 0) {
				byte[] data = new byte[(int)size];
				mInternalWriter.seek(0);
				mInternalWriter.write(data);
			}
		} catch (IOException e) {
